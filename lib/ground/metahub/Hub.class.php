...
class metahub_Hub {
	...
	public function create_node($trellis) {
		...
		if($node === null) {
			throw new HException(new HException("Could not find valid factory to create node of type " . _hx_string_or_null($trellis->name) . ".", null, null, _hx_anonymous(array("fileName" => "Hub.hx", "lineNumber" => 59, "className" => "metahub.Hub", "methodName" => "create_node"))));
		}
	...
	public function get_node($id) {
		if($id < 0 || $id >= $this->nodes->length) {
			throw new HException(new HException("There is no node with an id of " . _hx_string_rec($id, "") . ".", null, null, _hx_anonymous(array("fileName" => "Hub.hx", "lineNumber" => 67, "className" => "metahub.Hub", "methodName" => "get_node"))));
		}
	...
	public function run_code($code) {
		...
		if(!$result->success) {
			throw new HException(new HException("Error parsing code.", null, null, _hx_anonymous(array("fileName" => "Hub.hx", "lineNumber" => 103, "className" => "metahub.Hub", "methodName" => "run_code"))));
		}
		$match = $result;
